...
public class BusLocationPlotter extends MapViewOverlay {
    ...
    public void plotBuses() {
        ...
        if (selected!= null) {
            List<Bus> busList = selected.getBuses();
            for (Bus bus : busList) {
                OverlayItem item = new OverlayItem(bus.getDestination(), bus.getTime(), new GeoPoint(bus.getLatLon().getLatitude(), bus.getLatLon().getLongitude()));
                item.setMarker(busIconDrawable);
