...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
        ...
        for (Stop stop : StopManager.getInstance()) {
            if (rectangleContainsPoint(northWest, southEast, stop.getLocn())) {
            ...
            updateMarkerOfNearest(StopManager.getInstance().findNearestTo(new LatLon(currentLocation.getLatitude(), currentLocation.getLongitude())));
        }
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        if(nearestStnMarker != null){
            ...
            nearestStnMarker = this.getMarker(nearest);
